...
class SalarySlip(TransactionBase):
	...
	def validate(self):
		...
		self.payment_days_custom = (self.payment_days-self.additional_days_deduction)-self.absent_days
		self.calculate_net_pay()
		self.check_absent_and_lates()
		self.calculate_net_pay_again()
	...
	def check_absent_and_lates(self):
		basic = 0
		late_mins = 0
		late_deducation = 0
		early_mins = 0
		...
		absent_days = 0
		absent_deducation = 0
		self.standard_salary=0
		me = frappe.utils.getdate(str(self.start_date))
		no_of_days = calendar.monthrange(int(me.year), int(me.month))[1]
		period = int(frappe.utils.getdate(str(self.end_date)).day) - int(frappe.utils.getdate(str(self.start_date)).day)
		period += 1
		hours_for_month = 0
		exit_allowance_total = 0
		shifts = frappe.db.sql("""select at.shift, sf.total_hours, count(at.name), sf.early_exit_allowance_in_mins from `tabAttendance` at
			inner join `tabShift Type` sf on sf.name = at.shift
			where at.docstatus = 1 and at.employee = %s and 
			at.attendance_date between %s and %s group by at.shift""",(self.employee, self.start_date, self.end_date))
		for d in shifts:
			hours_for_month += d[1]*d[2]
			exit_allowance_total += d[3]*d[2]
		total_hours = hours_for_month/self.payment_days
		exit_allowance = exit_allowance_total/self.payment_days
		# frappe.throw(str(total_hours))
		# shift = frappe.db.get_value("Employee", self.employee, "default_shift")
		# if not shift: frappe.throw("Assign shift in employee {}".format(self.employee))
		# total_hours = frappe.db.get_value("Shift Type", shift, "total_hours")
		# exit_allowance = frappe.db.get_value("Shift Type", shift, "early_exit_allowance_in_mins")
		if self.earnings:
			for e in self.get("earnings"):
				if e.salary_component == "Basic":
					self.standard_salary += e.amount
					if not self.basic:
						self.basic = e.amount
					if self.payroll_frequency == "Bimonthly":
						basic = (self.basic/no_of_days)*period
						e.amount = basic
						e.done=1
					else:
						basic += e.amount
				if e.salary_component == "Medical Allowance":
					basic += e.amount
					self.standard_salary += e.amount
			if basic:
				lates = frappe.db.sql("""select sum(late_entry_mins) from `tabAttendance` where docstatus = 1 and employee = %s and 
					attendance_date between %s and %s and late_entry = 1 and status = 'Present'""",(self.employee, self.start_date, self.end_date))
				if lates: late_mins = lates[0][0]
				if late_mins:
					late_mins = late_mins//60
					self.late_entry_mins = late_mins
					total_mins1 = late_mins
					total_hours1 = 0
					while total_mins1 >= 60:
						total_hours1+=1
						total_mins1-=60
					self.late_entry_time = str(total_hours1).zfill(2)+":"+str(int(total_mins1)).zfill(2)
					salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Late Arrival Deduction"}, "name")
					if not salary_component: frappe.throw("Late Arrival Deduction component not found, Please create one.")
					rate_per_min = basic/self.payment_days/total_hours/60
					late_deducation = rate_per_min*late_mins
					if not self.deductions:
						self.append("deductions", {"salary_component": salary_component, "amount": late_deducation})
					else:
						exist = 1
						for d in self.deductions:
							if d.salary_component == salary_component:
								d.amount = late_deducation
								exist = 0
								break
						if exist:
							self.append("deductions", {"salary_component": salary_component, "amount": late_deducation})
				early = frappe.db.sql("""select sum(early_exit_mins) from `tabAttendance` where docstatus = 1 and employee = %s and 
					attendance_date between %s and %s and early_exit = 1 and status = 'Present'""",(self.employee, self.start_date, self.end_date))
				if early: early_mins = early[0][0]
				if early_mins:
					early_mins = early_mins//60
					early_mins = early_mins-exit_allowance
					if early_mins>0:
						self.early_exit_mins = early_mins
						total_mins1 = early_mins
						total_hours1 = 0
						while total_mins1 >= 60:
							total_hours1+=1
							total_mins1-=60
						self.early_exit_time = str(total_hours1).zfill(2)+":"+str(int(total_mins1)).zfill(2)
						salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Early Leave Deduction"}, "name")
						if not salary_component: frappe.throw("Early Leave Deduction component not found, Please create one.")
						rate_per_min = basic/self.payment_days/total_hours/60
						early_deducation = rate_per_min*early_mins
						if not self.deductions:
							self.append("deductions", {"salary_component": salary_component, "amount": early_deducation})
						else:
							exist = 1
							for d in self.deductions:
								if d.salary_component == salary_component:
									d.amount = early_deducation
									exist = 0
									break
							if exist:
								self.append("deductions", {"salary_component": salary_component, "amount": early_deducation})

				absents = frappe.db.sql("""select count(name) from `tabAttendance` where docstatus = 1 and employee = %s and 
					attendance_date between %s and %s and status = 'Absent'""",(self.employee, self.start_date, self.end_date))
				if absents: absent_days = absents[0][0]
				if absent_days:
					self.absent_days = absent_days
					salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Absent Deduction"}, "name")
					if not salary_component: frappe.throw("Absent Deduction component not found, Please create one.")
					per_day = basic/self.payment_days
					absent_deducation = per_day*absent_days
					if not self.deductions:
						self.append("deductions", {"salary_component": salary_component, "amount": absent_deducation})
					else:
						exist = 1
						for d in self.deductions:
							if d.salary_component == salary_component:
								d.amount = absent_deducation
								exist = 0
								break
						if exist:
							self.append("deductions", {"salary_component": salary_component, "amount": absent_deducation})

				overtime = frappe.db.sql("""select sum(overtime_mins) from `tabAttendance` where docstatus = 1 and employee = %s and 
					attendance_date between %s and %s and over_time = 1 and status = 'Present'""",(self.employee, self.start_date, self.end_date))
				if overtime: overtime_mins = overtime[0][0]
				if overtime_mins:
					overtime_mins = overtime_mins//60
					self.approved_overtime_mins = overtime_mins
					total_mins1 = overtime_mins
					total_hours1 = 0
					while total_mins1 >= 60:
						total_hours1+=1
						total_mins1-=60
					self.approved_overtime_time = str(total_hours1).zfill(2)+":"+str(int(total_mins1)).zfill(2)
					salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Approved Overtime"}, "name")
					if not salary_component: frappe.throw("Approved Overtime component not found, Please create one.")
					rate_per_min = basic/self.payment_days/total_hours/60
					overtime_earning = rate_per_min*overtime_mins
					if not self.earnings:
						self.append("earnings", {"salary_component": salary_component, "amount": overtime_earning})
					else:
						exist = 1
						for d in self.earnings:
							if d.salary_component == salary_component:
								d.amount = overtime_earning
								exist = 0
								break
						if exist:
							self.append("earnings", {"salary_component": salary_component, "amount": overtime_earning})

				overtime = frappe.db.sql("""select sum(total_secs) from `tabOvertime` where docstatus = 1 and employee = %s and 
					date between %s and %s""",(self.employee, self.start_date, self.end_date))
				if overtime: overtime_mins = overtime[0][0]
				if overtime_mins:
					overtime_mins = overtime_mins//60
					self.overtime_mins = overtime_mins
					total_mins1 = overtime_mins
					total_hours1 = 0
					while total_mins1 >= 60:
						total_hours1+=1
						total_mins1-=60
					self.overtime_time = str(total_hours1).zfill(2)+":"+str(int(total_mins1)).zfill(2)
					salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Overtime"}, "name")
					if not salary_component: frappe.throw("Overtime component not found, Please create one.")
					rate_per_min = basic/self.payment_days/total_hours/60
					overtime_earning = rate_per_min*overtime_mins
					if not self.earnings:
						self.append("earnings", {"salary_component": salary_component, "amount": overtime_earning})
						...
						if exist:
							self.append("earnings", {"salary_component": salary_component, "amount": overtime_earning})
				if self.additional_days_deduction:
					salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Additional Days Deduction"}, "name")
					if not salary_component: frappe.throw("Additional Days Deduction component not found, Please create one.")
					per_day = basic/self.payment_days
					absent_deducation = per_day*self.additional_days_deduction
					if not self.deductions:
						self.append("deductions", {"salary_component": salary_component, "amount": absent_deducation})
					else:
						exist = 1
				...
				# absents = frappe.db.sql("""select count(name) from `tabAttendance` where docstatus = 1 and employee = %s and 
				# 	attendance_date between %s and %s and status = 'Half Day'""",(self.employee, self.start_date, self.end_date))
				# if absents: absent_days = absents[0][0]
				# if absent_days:
				# 	salary_component = frappe.db.get_value("Salary Component", {"salary_component": "Half Day"}, "name")
				# 	if not salary_component: frappe.throw("Half Day component not found, Please create one.")
				# 	absent_days = absent_days/2
				# 	per_day = basic/self.payment_days
				# 	absent_deducation = per_day*absent_days
				# 	if not self.deductions:
				# 		self.append("deductions", {"salary_component": salary_component, "amount": absent_deducation, "reason": str(absent_days)+" total half days"})
				# 	else:
				# 		exist = 1
				# 		for d in self.deductions:
				# 			if d.salary_component == salary_component:
				# 				d.amount = absent_deducation
				# 				d.reason = str(absent_days)+" total half days"
				# 				exist = 0
				# 				break
				# 		if exist:
				# 			self.append("deductions", {"salary_component": salary_component, "amount": absent_deducation, "reason": str(absent_days)+" total half days"})
	...
	def calculate_net_pay_again(self):
		self.gross_pay = 0.0
		self.total_deduction = 0.0
		for d in self.earnings:
			self.gross_pay += d.amount
		for d in self.deductions:
			self.total_deduction += d.amount
	...
	def add_structure_components(self, component_type):
		...
		for struct_row in self._salary_structure_doc.get(component_type):
			...
			if amount and struct_row.statistical_component == 0 and struct_row.done == 0:
