...
class Employee(NestedSet):
	...
	def validate(self):
		...
		# shift_list = frappe.db.sql(""" select shift from `tabEmployee Shift` where parent = %s order by idx desc limit 1""", (self.name))
		# if shift_list:
		# 	for a in shift_list:
		# 		self.active_shift = a[0]
		# if self.active_shift:
		# 	s = frappe.db.sql(""" select next from `tabShift Type` where name = %s """, (self.active_shift))
		# 	self.check_out_nextday = s
