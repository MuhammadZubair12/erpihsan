...
def execute(filters=None):
	...
	for ss in salary_slips:
		row = [ss.name, ss.employee,ss.employee_number, ss.employee_name, doj_map.get(ss.employee), ss.branch, ss.department, ss.designation,
			ss.company, ss.start_date, ss.end_date, ss.leave_without_pay, ss.payment_days_custom]
...
def get_columns(salary_slips):
	...
	"""
	columns = [
		_("Salary Slip ID") + ":Link/Salary Slip:150",_("Employee") + ":Link/Employee:120",_("Employee No") + "::120", _("Employee Name") + "::140",
...
def get_conditions(filters):
	...
	if filters.get("employee_number"): conditions += " and employee_number like '%%{}%%'".format(filters.get("employee_number"))
